...
assertEqual(order.open_price, original_open_price)
assertEqual(order.status, OrderStatus.OPENED)
errid, result = UpdateOrderTags(order.uid, patch={'manager_mode': 'normal'}, remove_keys=['remove_me'],
                                expected_tag_ver=1)
assertEqual(errid, 0)
order = GetOrder(order.uid)
assertEqual(order.tags['manager_mode'], 'normal')
assertFalse('remove_me' in order.tags)
errid, result = SetOrderTags(order.uid, {'tag_ver': 2, 'manager_owner': 'set_only'})
assertEqual(errid, 0)
order = GetOrder(order.uid)
assertEqual(order.tags['tag_ver'], 2)
assertEqual(order.tags['manager_owner'], 'set_only')
assertFalse('score' in order.tags)
assertEqual(order.volume, original_volume)
assertEqual(order.stop_loss, original_stop_loss)
assertEqual(order.take_profit, original_take_profit)
assertEqual(order.open_price, original_open_price)
...
if open_result['command_uid']:
    assertEqual(order.comment, f"cuid#{open_result['command_uid']}|")
else:
    assertEqual(order.comment, f"uid#{open_result['order_uid']}|")
