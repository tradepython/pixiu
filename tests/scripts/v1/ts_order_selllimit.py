...
if len(ood) == 0:
    ...
    if len(pod) == 0:
        ...
        # failed
        price = Bid()
        errid, result = Sell(volume=0.01, type=OrderType.LIMIT, price=price, stop_loss=stop_loss,
    ...
    pass
ctime = Time()
if ctime == valid_selllimit_time:
    pod = GetPendingOrderUIDs()
    assertEqual(len(pod), 0)
    assertEqual(len(ood), 1)
    # order = GetOrder(result['order_uid'])
    # assertEqual(order.symbol, Symbol())
    # assertEqual(order.result['order_uid'], result['order_uid'])
